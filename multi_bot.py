    ...
    async def spam(ctx, *, args):
        ...
        async def loop():
            while True:
                ...
                await asyncio.sleep(delay)
        bot.spam_tasks[ctx.channel.id] = bot.spam_tasks[ctx.channel.id] = bot.spam_tasks[ctx.channel.id] = bot.spam_tasks[ctx.channel.id] = bot.spam_tasks[ctx.channel.id] = asyncio.create_task(loop())
